...
String wifiSSID = "";
String wifiPassword = "";
const char *mqttServer = "47.91.203.178"; // Onenet IP address
const int mqttPort = 6002;                // port number                               //variable of second data
...
boolean mqttConnect()
{
    ...
    mqttClient.connect(onenetDeviceID.c_str(), onenetProductID.c_str(), onenetPassword.c_str());
...
void sendPinValue()
{
    if (mqttClient.connected())
    {
            ...
            if (pinEnable[i] && pinReadWrite[i] == 0)
            {
                ...
                if (pinEventTriggerEnable[i])
                {
                    ...
                    case 0:
                        ...
                        {
                            mqtt_msg += ",\"" + pinTopic[i] + " EventTrigger\": 1";
                        }
                        break;
                    case 1:
                        if (value == pinEventTriggerValue[i])
        ...
        mqtt_msg += "}";
        const char *msgJson = mqtt_msg.c_str();
        ...
        mqttClient.publish("$dp", (uint8_t *)msg_buf, 3 + strlen(msgJson)); // Send data
